...
// Panel: GDEW1248Z95 : http://www.e-paper-display.com/products_detail/productId=408.html
...
GxEPD2_1248c::GxEPD2_1248c(int8_t sck, int8_t miso, int8_t mosi,
...
GxEPD2_1248c::GxEPD2_1248c(int8_t cs_m1, int8_t cs_s1, int8_t cs_m2, int8_t cs_s2,
                           int8_t dc1, int8_t dc2, int8_t rst1, int8_t rst2,
                           int8_t busy_m1, int8_t busy_s1, int8_t busy_m2, int8_t busy_s2) :
  GxEPD2_EPD(cs_m1, dc1, rst1, busy_m1, LOW, 10000000, WIDTH, HEIGHT, panel, hasColor, hasPartialUpdate, hasFastPartialUpdate),
  _sck(SCK), _miso(MISO), _mosi(MOSI), _dc1(dc1), _dc2(dc2), _rst1(rst1), _rst2(rst2),
...
GxEPD2_1248c::GxEPD2_1248c(int8_t cs_m1, int8_t cs_s1, int8_t cs_m2, int8_t cs_s2, int8_t dc, int8_t rst, int8_t busy) :
...
void GxEPD2_1248c::writeImage(const uint8_t* black, const uint8_t* color, int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
  ...
  S2.writeImagePart(0x10, black, 0, 0, w, h, x, y, w, h, invert, mirror_y, pgm);
  M2.writeImagePart(0x10, black, 0, 0, w, h, x - S2.WIDTH, y, w, h, invert, mirror_y, pgm);
  M1.writeImagePart(0x10, black, 0, 0, w, h, x, y - S2.HEIGHT, w, h, invert, mirror_y, pgm);
  S1.writeImagePart(0x10, black, 0, 0, w, h, x - M1.WIDTH, y - M2.HEIGHT, w, h, invert, mirror_y, pgm);
  if (color) {
    S2.writeImagePart(0x13, color, 0, 0, w, h, x, y, w, h, !invert, mirror_y, pgm);
    M2.writeImagePart(0x13, color, 0, 0, w, h, x - S2.WIDTH, y, w, h, !invert, mirror_y, pgm);
    M1.writeImagePart(0x13, color, 0, 0, w, h, x, y - S2.HEIGHT, w, h, !invert, mirror_y, pgm);
    S1.writeImagePart(0x13, color, 0, 0, w, h, x - M1.WIDTH, y - M2.HEIGHT, w, h, !invert, mirror_y, pgm);
  }
}
                                 ...
                                 int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
  ...
  S2.writeImagePart(0x10, black, x_part, y_part, w, h, x, y, w, h, invert, mirror_y, pgm);
  M2.writeImagePart(0x10, black, x_part, y_part, w, h, x - S2.WIDTH, y, w, h, invert, mirror_y, pgm);
...
{
  drawImage(bitmap, NULL, x, y, w, h, invert, mirror_y, pgm);
}

void GxEPD2_1248c::drawImagePart(const uint8_t bitmap[], int16_t x_part, int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,
                                int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
    drawImagePart(bitmap, NULL, x_part, y_part, w_bitmap, h_bitmap, x, y, w, h, invert, mirror_y, pgm);
}
...
const unsigned char GxEPD2_1248c::lut_20_LUTC_partial[] PROGMEM =
{
...
const unsigned char GxEPD2_1248c::lut_21_LUTWW_partial[] PROGMEM =
{
...
const unsigned char GxEPD2_1248c::lut_22_LUTKW_partial[] PROGMEM =
{
...
const unsigned char GxEPD2_1248c::lut_23_LUTWK_partial[] PROGMEM =
{
  0x91, 0x10, 0x10, 0x01, 0x08, 0x01,
  0x04, 0x06, 0x01, 0x06, 0x01, 0x05,
  0x84, 0x08, 0x01, 0x08, 0x01, 0x06,
...
const unsigned char GxEPD2_1248c::lut_24_LUTKK_partial[] PROGMEM =
{
...
const unsigned char GxEPD2_1248c::lut_25_LUTBD_partial[] PROGMEM =
{
  0x91, 0x10, 0x10, 0x01, 0x08, 0x01,
  0x04, 0x06, 0x01, 0x06, 0x01, 0x05,
  0x84, 0x08, 0x01, 0x08, 0x01, 0x06,
...
void GxEPD2_1248c::_InitDisplay()
{
  ...
  M1.writeData(0x2f);  //KW-3f   KWR-2F  BWROTP 0f BWOTP 1f
  S1.writeCommand(0x00);
  S1.writeData(0x2f);
  M2.writeCommand(0x00);
  M2.writeData(0x23); // reverse scan
  S2.writeCommand(0x00);
  S2.writeData(0x23); // reverse scan
  // POWER SETTING
  M1.writeCommand(0x01);
  M1.writeData(0x07);
  M1.writeData(0x17);	//VGH=20V,VGL=-20V
  M1.writeData(0x3F);	//VDH=15V
  M1.writeData(0x3F);  //VDL=-15V
  M1.writeData(0x0d);
  M2.writeCommand(0x01);
  M2.writeData(0x07);
  M2.writeData(0x17);	//VGH=20V,VGL=-20V
  M2.writeData(0x3F);	//VDH=15V
  M2.writeData(0x3F);  //VDL=-15V
  M2.writeData(0x0d);
  // booster soft start
  ...
  M1.writeCommand(0xE0);			//POWER SETTING
  M1.writeData(0x01);
  M2.writeCommand(0xE0);			//POWER SETTING
  M2.writeData(0x01);

  _writeCommandAll(0xE3);
  _writeDataAll(0x00);

  M1.writeCommand(0x82);
  M1.writeData(0x1c);
  M2.writeCommand(0x82);
  M2.writeData(0x1c);
...
GxEPD2_1248c::ScreenPart::ScreenPart(uint16_t width, uint16_t height, bool rev_scan, int8_t cs, int8_t dc) :
